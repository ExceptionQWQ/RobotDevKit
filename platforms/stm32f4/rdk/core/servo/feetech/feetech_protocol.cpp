...
bool FeetechProtocol::read_position(uint8_t id, int16_t* position, bool endian_reverse)
{
    ...
    if (endian_reverse) pos = endian_reverse16(pos);
    *position = pos;
    return ret;
}
...
{
    if (endian_reverse) position = endian_reverse16(position);
...
{
    if (endian_reverse) position = endian_reverse16(position);
    if (endian_reverse) speed = endian_reverse16(speed);
    uint8_t data[6];
...
uint16_t FeetechProtocol::endian_reverse16(uint16_t value)
{
