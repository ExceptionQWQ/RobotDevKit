...
ReliableBinaryTransfer::ReliableBinaryTransfer(SerialPort* serial_port, Crc8Calculator* crc8_calculator): SimpleBinaryTransfer(serial_port, crc8_calculator)
{
...
std::size_t ReliableBinaryTransfer::send_binary_once(uint8_t* data, std::size_t len)
{
    ...
    for (int try_cnt = 0; try_cnt < max_try_cnt * 6; ++try_cnt) {
