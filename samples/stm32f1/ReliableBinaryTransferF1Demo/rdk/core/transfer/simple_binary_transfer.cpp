...
SimpleBinaryTransfer::SimpleBinaryTransfer(SerialPort* serial_port, Crc8Calculator* crc8_calculator)
{
    this->serial_port = serial_port;
    this->crc8_calculator = crc8_calculator;
}
...
void SimpleBinaryTransfer::send_binary(uint8_t* data, std::size_t len)
{
    ...
    serial_port->write(send_buff, pkg_len);
    send_bytes += pkg_len;
}
    ...
    while (true) {
        ...
        switch (recv_status) {
            case RecvStatus::RecvFirstFrameStart:
                serial_port->read(recv_buff + pkg_len, 1, timeout);
            ...
            case RecvStatus::RecvBinary:
                while (binary_len) {
                    std::size_t recv_len = serial_port->read(recv_buff + pkg_len, binary_len, timeout);
                    pkg_len += recv_len;
                    binary_len -= recv_len;
                }
